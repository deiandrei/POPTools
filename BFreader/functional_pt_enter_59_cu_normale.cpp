...
int main(int argc, char** argv) {
	std::ifstream in1("test.in", std::ios::binary);
	...
	popbin::Entry& entry = arch->Entries[59];
	...
	bb->Read<uint4>(&uv_count); // UVs count
	...
	for (int i = 0; i < indices_count; ++i) {
		...
		bb->Read<int2>(&temp1); // wtf is this?
		bb->Read<int2>(&temp2); // also wtf is this...
